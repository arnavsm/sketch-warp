...
from ..models.correspondence_model import mask_outlier
...
from ..utils.metrics import AverageMeter, ProgressMeter, accuracy, proj_kps, compute_pck
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, args, writer):
    ...
    logger = Logger(log_interval=args.print_freq, log_file=os.path.join(args.log_dir, "training_log.txt"))
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.task == "encoder" or args.task == "both":
            ...
            if writer is not None:
                n_iters = epoch * len(train_loader) + i
                writer.add_scalar('train/clr_loss', clr_loss.item(), n_iters)
                writer.add_scalar('train/acc1', acc1[0], n_iters)
                writer.add_scalar('train/acc5', acc5[0], n_iters)
        ...
        if args.task == "estimator" or args.task == "both":
            ...
            with torch.cuda.amp.autocast():
                ...
                if args.sim_loss_weight != 0:
                    ...
                    if writer is not None:
                        writer.add_scalar('train/sim_loss', sim_loss.item(), n_iters)
                ...
                if args.syn_loss_weight != 0:
                    ...
                    if writer is not None:
                        writer.add_scalar('train/syn_loss', syn_loss.item(), n_iters)
                ...
                if args.con_loss_weight != 0:
                    ...
                    if writer is not None:
                        writer.add_scalar('train/con_loss', con_loss.item(), n_iters)
        ...
        if i % args.print_freq == 0:
            logger.on_batch_end(i, {
                'clr_loss': clr_loss.item() if 'clr_loss' in locals() else None,
                'sim_loss': sim_loss.item() if 'sim_loss' in locals() else None,
                'syn_loss': syn_loss.item() if 'syn_loss' in locals() else None,
                'con_loss': con_loss.item() if 'con_loss' in locals() else None,
                'acc1': acc1[0] if 'acc1' in locals() else None,
                'acc5': acc5[0] if 'acc5' in locals() else None,
            })
